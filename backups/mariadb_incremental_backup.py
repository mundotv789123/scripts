...
if not last_full_backup:
    ...
    path_to_backup = os.path.join(backup_dir, current_date.strftime('%d-%m-%Y_%H-%M-%S'))
    res = cur.execute('INSERT INTO backups (parent_id, path) VALUES (?, ?)', [last_backup['id'], path_to_backup])
    con.commit()
...
# expurgo
res = cur.execute(f"SELECT * FROM backups WHERE parent_id is null AND DATE(created_at, '+{days_rotate} day') <= CURRENT_DATE")
