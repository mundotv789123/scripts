...
#
#  Facilite a instalação do painel pterodactyl.io com este script.
#  O script instala dependências, configura o servidor web Nginx, baixa e instala o painel Pterodactyl juntamente com o Node Wings.
#  Ao executar o script, ele pedirá um domínio válido apontado para a máquina onde está sendo executado.
#  Ele irá gerar um certificado SSL pelo Certbot e, a partir daí, começará todo o processo de instalação.
#  O script foi testado no Ubuntu 20.04 e 22.04, mas roda em outros sistemas derivados.
#
#  Bugs e erros que podem ocorrer:
#
#    1. Ao executar em uma máquina já configurada para hospedagem web, o script pode interferir no funcionamento do site.
#    2. Caso executado em uma máquina com o Pterodactyl já instalado, ele tentará evitar conflitos, mas isso não quer dizer que não possa acontecer.
#    3. O script pergunta se você deseja criar o certificado SSL. Caso já tenha criado ou não deseje usar o Certbot, não confirme a geração do certificado.
...
while [ $confirm != "y" ]; do
    ...
    read domain
    if  [ "$SKIP_SSL" == "true" ] || nslookup $domain -c1 > /dev/null; then
...
function install_libs {
    ...
    # instalando recursos
    apt update
    apt-get -y install php8.3 php8.3-cli php8.3-gd php8.3-mysql php8.3-pdo php8.3-mbstring php8.3-tokenizer php8.3-bcmath php8.3-xml php8.3-fpm php8.3-curl php8.3-zip
...
"| sed -e "s/<domain>/${domain}/g" > /etc/nginx/sites-available/pterodactyl
    else
...
if [ ! -d /var/www/pterodactyl/public/phpmyadmin ]; then
    ...
    if [ $phpm_confirm = 'y' ]; then
        ...
        curl -o phpmyadmin.zip https://files.phpmyadmin.net/phpMyAdmin/5.2.3/phpMyAdmin-5.2.3-all-languages.zip
        unzip phpmyadmin.zip
        mv -iv phpMyAdmin-5.2.3-all-languages phpmyadmin
